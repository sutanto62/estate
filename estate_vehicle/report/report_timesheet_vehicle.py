...
class report_timesheet_vehicle(report_sxw.rml_parse):
    """
    Timesheet Vehicle Report shows all approved/correction/payslip activities.

    """

    def __init__(self, cr, uid, name, context):
        super(report_timesheet_vehicle, self).__init__(cr, uid, name, context)
        self.localcontext.update({

        })

    def set_context(self, objects, data, ids, report_type=None):
        upkeep_obj = self.pool.get('fleet.vehicle.timesheet')
        ...
        self.vehicle_id = data['form'].get('vehicle_id')
        self.employee_id = data['form'].get('employee_id')
        self.company_id = data['form'].get('company_id')
        self.maintenance_state_id = data['form'].get('maintenance_state_id')

        return super(report_timesheet_vehicle, self).set_context(objects, data, ids, report_type=report_type)

    def _get_timesheet(self, data):
        """
        :param data:
        :return: list of estate timesheet activity instance
        """
        upkeep_obj = self.pool.get('fleet.vehicle.timesheet')
        if self.vehicle_id and self.employee_id:
            ids = upkeep_obj.search(self.cr, self.uid, [('state', 'in', STATE),
                                                        ('date', '>=', self.date_start),
                                                        ('date', '<=', self.date_end),
                                                        ('vehicle_id', '=', self.vehicle_id[0])])
    ...
    def _get_timesheet_activity(self, data):
        """
        Get sorted active timesheet
        ...
        # Get timesheet
        for record in self._get_timesheet(data):
            for activities in record.timesheet_ids:
                vehicle_ids.append(activities.vehicle_id)
        res += set(vehicle_ids)

        # Get view activity
        for activity in set(vehicle_ids):
            parent = False
...
class wrapped_report_timesheet_vehicle(osv.AbstractModel):
    _name = 'report.estate_vehicle.report_timesheet_vehicle'
    _inherit = 'report.abstract_report'
    _template = 'estate_vehicle.report_timesheet_vehicle'
    _wrapped_report_class = report_timesheet_vehicle