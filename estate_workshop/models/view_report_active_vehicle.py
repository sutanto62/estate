...
class ViewSummaryVehicleStatus(models.Model):
    ...
    def init(self, cr):
        cr.execute("""create or replace view view_summary_vehicle_available as
                    select
                        k.id,
                        to_char(to_timestamp (k.bulan::text, 'MM'), 'Month') as month_log_text,
                        to_char(to_timestamp (k.tahun::text, 'YYYY'), 'YYYY') as year_log_text,
                        k.parent_id,
                        k.date_id,
                        vehicle_id,hke_ho,hke_non_ho,
                        tanggal1,
                        ...
                        somefuncname(k.bulan::integer,k.tahun::integer,k.vehicle_id,1) sbi_total_day_breakdown,
                        somefuncname(k.bulan::integer,k.tahun::integer,k.vehicle_id,2) sbi_total_day_available,
                        somefuncname(k.bulan::integer,k.tahun::integer,k.vehicle_id,3) sbi_total_day_standby,
                        somefuncname(k.bulan::integer,k.tahun::Integer,k.vehicle_id,4) sbi_total_day_hke
                    from view_summary_vehicle_status_detail	k""")